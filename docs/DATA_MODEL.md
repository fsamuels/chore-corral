...
| `endpoint`   | text, not null, **unique**           | The push service URL. Globally unique so a re-subscribing device upserts rather than accumulating stale rows; `CHECK`-constrained to the three known Web Push service origins this app supports (`fcm.googleapis.com`, `updates.push.services.mozilla.com`, `web.push.apple.com`) — see DECISIONS.md |
