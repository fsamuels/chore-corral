...
Not a table — a view joining `farm_memberships` to `auth.users`, exposing just enough of `auth.users` (id, email, Google profile name) to attribute `activity_log` entries and task completions to a person, plus the membership's `role`. `auth.users` itself is never exposed directly via PostgREST.
