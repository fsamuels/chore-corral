...
**Public routes**: every route requires an authenticated session by default (`@nuxtjs/supabase`'s `redirectOptions`), with two narrow exceptions listed in its `exclude` array — `/components-demo` (a static Vuetify sampler with no data access) and `/landing` (the public marketing page, SPEC.md — Authentication). `public/robots.txt` disallows crawling everything except `/landing`, since every other route either requires auth or serves no independently useful content to a crawler.
...
4. On the device, `public/sw.js` — a deliberately **push-only** service worker (no offline caching; that remains its own roadmap item) — shows the notification and, on tap, focuses/opens the app at the chore's page. The notification's URL carries the chore's farm id (`?farm=<id>`); if a different farm is active in that browser, the task page switches to it before loading the chore — otherwise the farm-scoped lookup would report a chore that exists (just in another farm) as not found.
