...
- **Done (2026-07-06): New Tags page** — `/tags` lists every tag on the active farm with a usage count (`listTagsWithCounts` in `app/services/tags.ts`), read-only, alphabetically sorted, matching `/categories`' empty/loading/error state patterns. A nav drawer entry was added alongside "Categories".
...
- **Done (2026-07-06): Standardize tag naming convention** — resolved to **lowercase, spaces allowed** (e.g. "fence repair", not "fence-repair" or "Fence Repair"). `resolveTags` (`app/services/tags.ts`) now normalizes candidate names (trim, lowercase, collapse internal whitespace) before matching/creating, and a new migration (`supabase/migrations/20260706140000_normalize_tag_names.sql`) backfills existing rows, merging any tags that collide once normalized rather than just renaming them. See DECISIONS.md for the tag-matching background this extends and the merge algorithm's reasoning.
