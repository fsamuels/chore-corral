...
- **Public landing page (`/landing`).** The default destination for a signed-out visit to `/` or any other auth-gated route — a marketing page (not gated by the Supabase auth middleware) whose sign-in button drives into the same self-serve Google OAuth flow described above, rather than a separate waitlist/signup mechanism. It sets explicit expectations about the app's early-access state (see ROADMAP.md/STATUS.md) since there's no farm-cleanup path for a visitor who creates a farm and abandons it. Every other route stays private; `public/robots.txt` disallows crawling anything except this one page. `/login` still exists as a standalone, non-default sign-in page reachable directly.
