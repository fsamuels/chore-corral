...
## Tag names normalized to lowercase on write, with a merge-based backfill

Resolved 2026-07-06, closing out the M9 backlog item. The earlier "Tag matching" entry above made case-insensitive matching an application-layer concern rather than a database one, but deliberately preserved whichever casing was typed first — so "Fence" and "fence" resolved to the same row without ever settling on one convention. That's no longer good enough on its own: near-identical tags that differ only in whitespace (e.g. "Fence Repair" vs. "Fence Repair") weren't caught at all, and a farm's tag list would accumulate whatever mix of casing happened to be typed first. The resolved convention is **lowercase, spaces allowed** (e.g. "fence repair", not "fence-repair" or "Fence Repair") — `resolveTags` (`app/services/tags.ts`) now normalizes every candidate name (trim, lowercase, collapse internal whitespace to single spaces) before matching or creating, so the stored name itself is always the normalized form rather than whatever casing was first seen.

That change only affects tags created from here on; tags already in the database needed a backfill, which is where the harder problem lives. A straightforward `update tags set name = normalize(name)` isn't safe: `unique(farm_id, name)` is case-sensitive today, so two previously-distinct rows within the same farm (e.g. "Fence" and "fence ") can normalize to the same string and collide on that constraint mid-update. The backfill migration (`supabase/migrations/20260706140000_normalize_tag_names.sql`) therefore merges rather than renames: it groups existing tags by `(farm_id, normalized_name)`, picks one canonical row per group (oldest by `created_at`, tag id as a tiebreaker), repoints `task_tags` rows from the losing tag ids to the canonical id, deletes the losing tag rows, and only then renames the canonical row to its normalized form. Repointing `task_tags` has its own collision to avoid: a task can already be tagged with two tags that are about to merge (e.g. tagged with both "Fence" and "fence"), which would collide on `task_tags`' `(task_id, tag_id)` composite PK once both point at the canonical id — the migration dedupes `task_tags` rows per `(task_id, canonical_id)` pair first (keeping an arbitrary survivor) before doing the repoint, which correctly handles both "task already tagged with the canonical tag" and "task tagged with two different losing tags" cases uniformly.

This migration hasn't been applied to or verified against the hosted project — same sandboxed-environment limitation as every other migration in this project's history (no local Postgres harness is checked in). `supabase db push` plus a spot-check of a farm with real tag data is a manual follow-up step. The demo farm's 36 seeded tags (`supabase/seeds/clarksons_farm_sample_data.sql`) are already lowercase and don't collide under this normalization, so they're left as-is; several use hyphens instead of spaces (e.g. "spring-barley") which the resolved convention doesn't require changing (only lowercase + whitespace collapsing) — reworking them to space-separated form, if wanted, is a follow-up, not part of this change.
