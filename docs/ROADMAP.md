...
- **In-place task editing** — **In progress (started 2026-07-09).** Replace the separate `/tasks/:id/edit` page with per-field inline editing on the task View page, each field saving immediately on commit (the pattern the View page's status dropdown already uses). Chosen over an edit dialog and a whole-page edit-mode toggle — see DECISIONS.md. Building incrementally: (1) chip menus for priority, due date, category, and estimated time; (2) title and notes; (3) tags; (4) location; (5) photos/shopping list/tools editable directly on the View page; (6) retire the Edit page (moving Delete onto the View page). The Edit page stays until the View page reaches full parity.
