...
- **Chore reminders via push notifications** — **Done (2026-07-19).** Set any number of date+time reminders on a chore; when one comes due, every farm member's push-subscribed devices get a Web Push notification (works with the app closed, including installed-PWA iOS 16.4+). Server-scheduled — pg_cron + a `send-reminders` Edge Function — since the web platform can't schedule local notifications; the service worker this added is deliberately push-only, leaving the offline-caching roadmap item below untouched. See DECISIONS.md.
