...
- **Chore reminders via push notifications** — **Done (2026-07-19).** Set any number of date+time reminders on a chore; when one comes due, every farm member's push-subscribed devices get a Web Push notification (works with the app closed, including installed-PWA iOS 16.4+). Server-scheduled — pg_cron + a `send-reminders` Edge Function — since the web platform can't schedule local notifications; the service worker this added is deliberately push-only, leaving the offline-caching roadmap item below untouched. Fired reminders can be snoozed 10 min / 1 hr, from action buttons on the notification itself (Android/desktop) or from the chore's Reminders card (the iOS path — iOS web push has no action buttons). See DECISIONS.md.
...
- **Named/saved locations** — **Done (2026-07-09).** Farms can define named, reusable point locations (e.g. "Shop", "Front Barn") on a new `/locations` CRUD page. Tasks pick a defined location or keep the existing freeform `LocationPicker` pin — never both, enforced by a DB check constraint (`tasks_location_xor_pin`) and an app-layer guard — freeform pins were kept, not replaced, since ad-hoc unnamed spots are still useful. The `/map` page renders labeled location markers and places location-linked tasks at their location's coordinates; deleting a location is blocked while any active task still references it, mirroring the `categories` delete guard. Natural predecessor to boundary polygons below.
