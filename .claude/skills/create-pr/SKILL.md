...
3. **Update documentation to match the changes.** Check each of these and edit only what's actually stale:
   - **[docs/STATUS.md](../../../docs/STATUS.md)** — always review this one. Update "Last updated" to today's date, "Current Phase", the Milestone Progress table (mark a milestone row "In Progress" or "Done" if this branch's work matches its done-state in MILESTONES.md), "Known Issues", and "Next Steps". This file must reflect reality *after* merge, not before.
